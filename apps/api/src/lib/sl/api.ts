...
const deviationsSchema = z.array(
    z.object({
        ...
        categories: z.array(z.object({ group: z.string(), type: z.string() })).optional()
    }),
);

const slCache = new TTLCache({ ttl: 1000 * 60 * 5 }); // 5 minutes

export async function getSlDepartures(siteId: string): Promise<z.infer<typeof departuresSchema> | null> {
    const cachedResponse = slCache.get(siteId) as z.infer<typeof departuresSchema> | undefined;
    if (cachedResponse) return cachedResponse;

    // Cache miss, so we fetch from the API
    const fetchRes = await fetch(`https://transport.integration.sl.se/v1/sites/${siteId}/departures?forecast=60`);
...
export async function getSlDeviations(): Promise<z.infer<typeof deviationsSchema> | null> {
    const cacheKey = 'deviations';
    const cachedResponse = slCache.get(cacheKey) as z.infer<typeof deviationsSchema> | undefined;
    if (cachedResponse) return cachedResponse;

    // Cache miss, so we fetch from the API
    const fetchRes = await fetch(`https://deviations.integration.sl.se/v1/messages?future=false`);
    if (!fetchRes.ok) return null;
 ...
 */
export async function getFilteredSlDeviations(lineDesignation: string, stopAreaId?: number): Promise<z.infer<typeof deviationsSchema>> {
